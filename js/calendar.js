...
class CalendarManager {
  ...
  renderConsolidatedDayShifts(dayData, storeName) {
    ...
    const shiftNames = ['1º Turno (Manhã)', '2º Turno (Intermediário)', '3º Turno (Noite)'];
    let html = '';
    ...
    dayData.slots.forEach((slot, sIdx) => {
      ...
      if (workers.length === 0) {
        if (sIdx === 1) return; // Omitir 2º Turno quando vazio
        html += `
      ...
      l1Html = `
        ...
        <div style="background: #ffffff; padding: 0.4rem 0.6rem; border-radius: 6px; border: 1px solid #cbd5e1;">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 4px; align-items: center;">
            ...
            <span class="slot-slash" style="color: #64748b; font-weight: 700; margin: 0 4px; align-self: center;">/</span>
  ...
  shareTodayWhatsApp() {
    ...
    if (dayData && dayData.slots) {
      ...
      dayData.slots.forEach((s, idx) => {
        ...
        if (workers.length === 0) {
          if (idx === 1) return; // Omitir 2º turno vazio no WhatsApp
          l2Text += `  • *${shiftLabels[idx]}*: Sem escala\n`;
        } else {
          const names = workers.map(w => w.employeeName + (w.color === 'GREEN' ? ' (Folga)' : '')).join(' / ');
          l2Text += `  • *${shiftLabels[idx]}*: ${names}\n`;
        }
      });
    } else {
